// Board/Device FROST DKG
// UART Communication - Ready to compile
// Provides detailed information about who misbehaved and why
...
typedef struct {
    int result;  // 1 = honest, 0 = dishonest, -1 = error
    const char* misbehavior_type;
    const char* reason;
    uint32_t culprit;
    uint32_t victim;
} board_blame_verdict_t;
...
// STATIC BUFFERS for blaming verification
...
// ENHANCED BLAME PROOF VERIFICATION WITH DETAILED DIAGNOSTICS
static int verify_blame_proof_with_details(const blame_proof_t *proof,
                                           const secp256k1_pubkey *receiver_pubkey,
                                           board_blame_verdict_t *verdict) {
    LOG_INF("");
    LOG_INF("=== BLAME PROOF VERIFICATION ===");
    LOG_INF("Generator: %u | Receiver: %u", proof->generator_index, proof->receiver_index);
    LOG_INF("");
    
    verdict->culprit = proof->generator_index;
    verdict->victim = proof->receiver_index;
    
    // Step 1: Verify c1 = r*G
    LOG_INF("Step 1: Verifying ephemeral key (c1 = r*G)");
    
    if (!secp256k1_ec_pubkey_create(ctx, &verify_computed_c1, proof->ephemeral_key)) {
        LOG_ERR("*** VERIFICATION FAILED ***");
        LOG_ERR("Error: Cannot create point from ephemeral key");
        LOG_ERR("Culprit: Participant %u (Generator)", proof->generator_index);
        LOG_ERR("Issue: Invalid ephemeral key provided");
        verdict->result = -1;
        verdict->misbehavior_type = "INVALID_EPHEMERAL_KEY";
        verdict->reason = "Cannot form valid EC point";
        return -1;
    }
    ...
    if (!secp256k1_ec_pubkey_serialize(ctx, verify_c1_serialized, &len, &verify_computed_c1,
        ...
        LOG_ERR("*** VERIFICATION FAILED ***");
        LOG_ERR("Error: Cannot serialize c1");
        verdict->result = -1;
        verdict->misbehavior_type = "SERIALIZATION_ERROR";
        verdict->reason = "Internal error";
        return -1;
    }
    
    if (memcmp(verify_c1_serialized, proof->c1_serialized, 33) != 0) {
        LOG_ERR("*** VERIFICATION FAILED ***");
        LOG_ERR("Error: Ephemeral key mismatch!");
        LOG_ERR("Computed c1: %02x%02x%02x%02x...", 
                verify_c1_serialized[0], verify_c1_serialized[1],
                verify_c1_serialized[2], verify_c1_serialized[3]);
        LOG_ERR("Proof c1:    %02x%02x%02x%02x...",
                proof->c1_serialized[0], proof->c1_serialized[1],
                proof->c1_serialized[2], proof->c1_serialized[3]);
        LOG_ERR("");
        LOG_ERR("CULPRIT: Participant %u (Generator) - DISHONEST", proof->generator_index);
        LOG_ERR("Reason: Provided wrong ephemeral key");
        LOG_ERR("Impact: Cannot trust this proof");
        
        verdict->result = -1;
        verdict->misbehavior_type = "EPHEMERAL_KEY_MISMATCH";
        verdict->reason = "Generator lied about ephemeral key";
        verdict->culprit = proof->generator_index;
        return -1;
    }
    
    LOG_INF("  OK: Ephemeral key verified");
    
    // Step 2: Recompute shared secret
    LOG_INF("Step 2: Recomputing shared secret (ECDH)");
    
    if (!secp256k1_ecdh(ctx, verify_shared_secret, receiver_pubkey, proof->ephemeral_key,
                        secp256k1_ecdh_hash_function_sha256, NULL)) {
        LOG_ERR("*** VERIFICATION FAILED ***");
        LOG_ERR("Error: ECDH computation failed");
        verdict->result = -1;
        verdict->misbehavior_type = "ECDH_FAILURE";
        verdict->reason = "Cannot compute shared secret";
        return -1;
    }
    
    LOG_INF("  OK: Shared secret: %02x%02x%02x%02x...", 
            verify_shared_secret[0], verify_shared_secret[1],
            verify_shared_secret[2], verify_shared_secret[3]);
    
    // Step 3: Decrypt share
    LOG_INF("Step 3: Decrypting share (plaintext = c2 XOR secret)");
    
    for (int i = 0; i < SECRET_VALUE_SIZE; i++) {
        verify_decrypted_value[i] = proof->c2[i] ^ verify_shared_secret[i];
    }
    
    LOG_INF("  OK: Decrypted: %02x%02x%02x%02x...",
            verify_decrypted_value[0], verify_decrypted_value[1],
            verify_decrypted_value[2], verify_decrypted_value[3]);
    
    // Step 4: Check for corrupted ciphertext
    LOG_INF("Step 4: Checking for corrupted ciphertext");
    ...
    if (c2_all_zero || c2_all_ff) {
        LOG_ERR("");
        LOG_ERR("*** MISBEHAVIOR DETECTED ***");
        LOG_ERR("Type: CORRUPTED_CIPHERTEXT");
        LOG_ERR("Pattern: All bytes are %s", c2_all_zero ? "0x00" : "0xFF");
        LOG_ERR("");
        LOG_ERR("CULPRIT: Participant %u (Generator) - DISHONEST", proof->generator_index);
        LOG_ERR("VICTIM:  Participant %u (Receiver) - HONEST", proof->receiver_index);
        LOG_ERR("");
        LOG_ERR("Analysis:");
        LOG_ERR("  - Proper ElGamal never produces this pattern");
        LOG_ERR("  - Generator sent deliberately corrupted data");
        LOG_ERR("  - Receiver correctly identified the problem");
        
        verdict->result = 0;
        verdict->misbehavior_type = "CORRUPTED_CIPHERTEXT";
        verdict->reason = "Generator sent invalid encrypted data";
        verdict->culprit = proof->generator_index;
        verdict->victim = proof->receiver_index;
        return 0;
    }
    
    LOG_INF("  OK: Ciphertext appears valid");
    
    // Step 5: Validate share value
    LOG_INF("Step 5: Validating share value");
    ...
    if (is_zero) {
        LOG_ERR("");
        LOG_ERR("*** MISBEHAVIOR DETECTED ***");
        LOG_ERR("Type: ZERO_VALUE_SHARE");
        LOG_ERR("Error: Share decrypts to all zeros");
        LOG_ERR("");
        LOG_ERR("CULPRIT: Participant %u (Generator) - DISHONEST", proof->generator_index);
        LOG_ERR("VICTIM:  Participant %u (Receiver) - HONEST", proof->receiver_index);
        LOG_ERR("");
        LOG_ERR("Analysis:");
        LOG_ERR("  - Valid FROST shares cannot be zero");
        LOG_ERR("  - Generator encrypted invalid value");
        LOG_ERR("  - Receiver was right to reject this");
        
        verdict->result = 0;
        verdict->misbehavior_type = "ZERO_VALUE_SHARE";
        verdict->reason = "Generator encrypted zero value";
        verdict->culprit = proof->generator_index;
        verdict->victim = proof->receiver_index;
        return 0;
    }
    ...
    if (validation_result == 0) {
        LOG_ERR("");
        LOG_ERR("*** MISBEHAVIOR DETECTED ***");
        LOG_ERR("Type: INVALID_SCALAR_VALUE");
        LOG_ERR("Error: Share is not a valid secp256k1 scalar");
        LOG_ERR("");
        LOG_ERR("CULPRIT: Participant %u (Generator) - DISHONEST", proof->generator_index);
        LOG_ERR("VICTIM:  Participant %u (Receiver) - HONEST", proof->receiver_index);
        LOG_ERR("");
        LOG_ERR("Analysis:");
        LOG_ERR("  - Value cannot form valid EC point");
        LOG_ERR("  - Not in valid range for secp256k1");
        LOG_ERR("  - Generator sent mathematically invalid data");
        
        verdict->result = 0;
        verdict->misbehavior_type = "INVALID_SCALAR";
        verdict->reason = "Value not in valid secp256k1 range";
        verdict->culprit = proof->generator_index;
        verdict->victim = proof->receiver_index;
        return 0;
    }
    
    LOG_INF("  OK: Share is valid scalar");
    
    // All checks passed - generator is honest
    LOG_INF("");
    LOG_INF("*** VERIFICATION COMPLETE ***");
    LOG_INF("");
    LOG_INF("VERDICT: Generator is HONEST");
    LOG_INF("CULPRIT: Participant %u (Receiver) - Made FALSE ACCUSATION", proof->receiver_index);
    LOG_INF("VICTIM:  Participant %u (Generator) - HONEST", proof->generator_index);
    LOG_INF("");
    LOG_INF("Analysis:");
    LOG_INF("  - Share decrypts to valid value");
    LOG_INF("  - ElGamal encryption was correct");
    LOG_INF("  - Receiver's accusation was false");
    LOG_INF("  - Generator proven innocent");
    
    verdict->result = 1;
    verdict->misbehavior_type = "FALSE_ACCUSATION";
    verdict->reason = "Receiver falsely accused honest generator";
    verdict->culprit = proof->receiver_index;
    verdict->victim = proof->generator_index;
...
static int handle_blame_proof_request(const blame_proof_request_t* request) {
    ...
    if (send_message(MSG_TYPE_BLAME_PROOF_RESPONSE, &proof, sizeof(proof)) != 0) {
...
// Static storage for test structures to avoid stack overflow
static blame_proof_t test_proof_storage;
static board_blame_verdict_t verdict_storage;

static void run_board_blaming_tests(void) {
    LOG_INF("");
    LOG_INF("╔═══════════════════════════════════════════════════════╗");
    LOG_INF("║      BOARD BLAMING PROTOCOL TEST SUITE               ║");
    LOG_INF("║   Independent verification with detailed reporting   ║");
    LOG_INF("╚═══════════════════════════════════════════════════════╝");
    LOG_INF("");
    ...
    LOG_INF("Test Setup:");
    LOG_INF("  Stored ephemeral keys: %u", num_ephemeral_keys);
    LOG_INF("  ElGamal public keys:   %u", num_elgamal_pubkeys);
    LOG_INF("  Context: DKG completed successfully");
    LOG_INF("");
    
    // Find ephemeral keys with different generator and receiver
    int test_key_index = -1;
    for (uint32_t i = 0; i < num_ephemeral_keys; i++) {
        if (ephemeral_keys[i].generator_index != ephemeral_keys[i].receiver_index) {
            test_key_index = i;
            break;
        }
    }
    ...
    if (test_key_index == -1) {
        LOG_WRN("No cross-participant ephemeral keys found");
        LOG_WRN("Using same-participant key for demonstration");
        test_key_index = 0;
    }
    
    uint32_t test_generator = ephemeral_keys[test_key_index].generator_index;
    uint32_t test_receiver = ephemeral_keys[test_key_index].receiver_index;
    
    LOG_INF("Using: Generator=%u, Receiver=%u", test_generator, test_receiver);
    if (test_generator == test_receiver) {
        LOG_WRN("  Note: Using same participant (generator=receiver)");
        LOG_WRN("  Ideally tests should use different participants");
    }
    LOG_INF("");
    ...
    // TEST 1: Verify Valid Share (Honest Generator)
    LOG_INF("┌───────────────────────────────────────────────────────┐");
    LOG_INF("│ TEST 1: Verifying Honest Generator                   │");
    LOG_INF("│ Goal: Detect false accusations                       │");
    LOG_INF("└───────────────────────────────────────────────────────┘");
    LOG_INF("");
    
    // Use static storage to avoid stack overflow
    test_proof_storage.generator_index = ephemeral_keys[test_key_index].generator_index;
    test_proof_storage.receiver_index = ephemeral_keys[test_key_index].receiver_index;
    memcpy(test_proof_storage.ephemeral_key, ephemeral_keys[test_key_index].ephemeral_key, 32);
    memcpy(test_proof_storage.c1_serialized, ephemeral_keys[test_key_index].c1_serialized, 33);
    memcpy(test_proof_storage.c2, ephemeral_keys[test_key_index].c2, SECRET_VALUE_SIZE);
    
    int result1 = verify_blame_proof_with_details(&test_proof_storage, receiver_pubkey, &verdict_storage);
    
    LOG_INF("");
    LOG_INF("=== TEST 1 RESULT ===");
    if (result1 == 1) {
        LOG_INF("Status: PASSED");
        LOG_INF("Finding: %s", verdict_storage.misbehavior_type);
        LOG_INF("Honest party: Participant %u", verdict_storage.victim);
        LOG_INF("Dishonest party: Participant %u", verdict_storage.culprit);
        LOG_INF("Explanation: %s", verdict_storage.reason);
    } else {
        LOG_ERR("Status: FAILED (unexpected result)");
    }
    LOG_INF("");
    
    k_msleep(2000);
    
    // TEST 2: Detect Corrupted Share (Dishonest Generator)
    LOG_INF("┌───────────────────────────────────────────────────────┐");
    LOG_INF("│ TEST 2: Detecting Dishonest Generator                │");
    LOG_INF("│ Goal: Identify invalid shares                        │");
    LOG_INF("└───────────────────────────────────────────────────────┘");
    LOG_INF("");
    
    // Reuse static storage for test 2
    test_proof_storage.generator_index = ephemeral_keys[test_key_index].generator_index;
    test_proof_storage.receiver_index = ephemeral_keys[test_key_index].receiver_index;
    memcpy(test_proof_storage.ephemeral_key, ephemeral_keys[test_key_index].ephemeral_key, 32);
    memcpy(test_proof_storage.c1_serialized, ephemeral_keys[test_key_index].c1_serialized, 33);
    memset(test_proof_storage.c2, 0xFF, SECRET_VALUE_SIZE);  // Corrupt c2
    
    LOG_INF("Simulating corrupted c2 (all 0xFF)...");
    LOG_INF("");
    
    int result2 = verify_blame_proof_with_details(&test_proof_storage, receiver_pubkey, &verdict_storage);
    
    LOG_INF("");
    LOG_INF("=== TEST 2 RESULT ===");
    if (result2 == 0) {
        LOG_INF("Status: PASSED");
        LOG_INF("Finding: %s", verdict_storage.misbehavior_type);
        LOG_INF("Dishonest party: Participant %u", verdict_storage.culprit);
    ...
    // Summary
    LOG_INF("╔═══════════════════════════════════════════════════════╗");
    LOG_INF("║            BOARD TESTS COMPLETE                       ║");
    LOG_INF("╚═══════════════════════════════════════════════════════╝");
    LOG_INF("");
    LOG_INF("Summary:");
    LOG_INF("  Test 1: %s - False accusations detected",
            result1 == 1 ? "PASSED" : "FAILED");
    LOG_INF("  Test 2: %s - Dishonest generator detected",
            result2 == 0 ? "PASSED" : "FAILED");
    LOG_INF("");
    LOG_INF("Capabilities Demonstrated:");
    LOG_INF("  ✓ Independent verification without coordinator");
    LOG_INF("  ✓ Detailed identification of misbehavior");
    LOG_INF("  ✓ Clear reporting of who misbehaved and why");
    LOG_INF("  ✓ Full cryptographic accountability");
    LOG_INF("");
}
...
static int process_received_message(void) {
    ...
    switch (current_header.msg_type) {
        ...
        case MSG_TYPE_BLAME_PROOF_REQUEST:
            if (current_header.payload_len >= sizeof(blame_proof_request_t)) {
                return handle_blame_proof_request(
                    (blame_proof_request_t*)payload_buffer);
            }
            return -1;
            
        case MSG_TYPE_BLAME_TEST_SCENARIO1:
            return handle_blame_test_scenario1();
            
        case MSG_TYPE_BLAME_TEST_SCENARIO2:
            return handle_blame_test_scenario2();
...
int main(void) {
    LOG_INF("FROST DKG Device Starting - ENHANCED BLAMING DIAGNOSTICS");
    LOG_INF("Shares are encrypted before sending, never in plaintext");
    LOG_INF("Includes detailed blaming for dispute resolution");
    ...
    LOG_INF("Enhanced: Detailed misbehavior diagnostics");
